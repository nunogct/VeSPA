...
always @(posedge clk) begin
    ...
    /*else if(i_req)begin    
        ...
        case (state)
            ...
            s_extra: state <= s_decode;
            //s_fetch: state <= (i_req) ? s_extra : s_decode;
...
assign pc_in_sel = (i_req || opcode == `RETI) ? 2'b00 :
                  (opcode == `JMP && state == s_fetch) ? 2'b01 :
                  (checkcond && (opcode == `BXX)) ? 2'b10 :
                  (state == s_fetch) ? 2'b11: 'hzz; 

assign ram_addr_in_sel = (opcode == `LD || opcode == `ST) ? 1'b0 :
                         (opcode == `LDX || opcode == `STX) ? 1'b1 : 'hz;

assign ir_load = (state == s_fetch) ? 1'b1 : 1'b0;                             
//assign ir_load = (state == s_fetch || state == s_extra) ? 1'b1 : 1'b0;

assign mem_write_en = (state == s_st || state == s_stx) ? 1'b1 : 1'b0;

assign pc_load = (state == s_fetch || (s_execute && i_req)) ? 1'b1 : 1'b0;
...
assign s_execute = (state == s_nop || state == s_add || state == s_sub || state == s_or ||
		 state == s_and || state == s_not || state == s_xor || 
         state == s_cmp || state == s_bxx || state == s_jmp || 
		state == s_ld || state == s_ldi || state == s_ldx ||
		state == s_st || state == s_stx || state == s_hlt) ? 1'b1 : 1'b0;
...
assign i_ack = (state == s_fetch && i_req) ? 1'b1 : 1'b0; 
